    ...
    public function applyCoupon(Request $req)
{
    ...
    if (!$coupon) {
        // ❌ Invalid coupon, flash message for popup
        return back()->with('error', 'Your coupon is not valid.');
    }

    // ✅ Save coupon in session with safe defaults
    session()->put('coupon', [
        'id' => $coupon->id,
        'code' => $coupon->code,
    ...
    public function process(Request $req)
{
    ...
    if (!empty($coupon)) {
        ...
        $type = $coupon['type'] ?? 'fixed';  // ✅ default 'fixed'
        $value = $coupon['value'] ?? 0;      // ✅ default 0
...
if (in_array($req->payment_method, ['card', 'razorpay'])) {
    ...
    try {
        ...
        foreach ($items as $item) {
            ...
            $image = $item->product->featured_image ?? ($item->product->images->first()->path ?? 'images/default.jpg');

            $order->items()->create([
                'product_id' => $item->product->id,
                'quantity' => $item->quantity ?? 1,
                'product_name' => $item->product_name ?? $item->product->title ?? $item->product->name,
                'product_image' => $image,
